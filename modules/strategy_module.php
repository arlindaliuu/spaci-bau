<?php 
    ...
    for( $i = 0; $i < count($modules); $i++){
        if($modules[$i]['acf_fc_layout'] == 'strategy_module'){
            $modules = $modules[$i];
        }
    }
    $strategy_title_1 = $modules['strategy_title_1'];
    $strategy_content_1 = $modules['strategy_content_1'];
    $strategy_button_1 = $modules['strategy_button_1'];
    $strategy_title_2 = $modules['strategy_title_2'];
    $strategy_content_2 = $modules['strategy_content_2'];
    $strategy_content_3 = $modules['strategy_content_3'];
    $strategy_title_3 = $modules['client_satisfaction']['strategy_title_3'];
    $strategy_and_planning_title = $modules['client_satisfaction']['strategy_and_planning_title'];
    $strategy_and_planning_content = $modules['client_satisfaction']['strategy_and_planning_content'];
    $client_satisfaction_title = $modules['client_satisfaction']['client_satisfaction_title'];
    $client_satisfaction_content = $modules['client_satisfaction']['client_satisfaction_content'];
?>
<section class="bg-blue-50 min-h-[500px] w-full">
    <div class="targetDiv animate-duration-[3000ms] rounded-lg overflow-hidden -mt-10 z-20 min-h-[500px] mx-10 relative max-w-mdm lg:mx-auto grid grid-cols-1 lg:grid-cols-3 shadow-2xl">
        <div class="lg:col-span-1 bg-orange-500 py-20 px-4 lg:px-12 grid gap-4 justify-items-center">
            ...
            <a href="/uber-uns" class="border rounded-full border-black-800 duration-300 hover:scale-110 font-semibold bg-black-800 text-white px-4 py-2 flex items-center h-fit"><?php echo $strategy_button_1 ?></a>
        </div>
